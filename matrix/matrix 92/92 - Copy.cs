    ...
    class Program
    {
        ...
        static int[,] ResetToZero(int[,] matrix)
        {
            ...
            for (int i = 0; i <= matrix.GetLength(0) / 2; i++)
            {
                ...
                //{
                //    for (int j = i + 1; j < matrix.GetLength(0) - i - 1; j++)
                //    {
                //        newMatrix[j, i] = 0;
                //        currentIndexOfColumn++;
                //
                ...
                for (int k = currentIndexOfColumn; k < matrix.GetLength(0); k++)
                {
                    k = currentIndexOfColumn;
                    newMatrix[k, i] = matrix[k, i];
                    currentIndexOfColumn++;

                    while (currentIndexOfColumn >= i + 1 && currentIndexOfColumn < matrix.GetLength(0) - i - 1)
                    {
                        for (int n = i + 1; n < matrix.GetLength(0) - i - 1; n++)
                        {
                            newMatrix[n, i] = 0;
                            currentIndexOfColumn++;
                        }
